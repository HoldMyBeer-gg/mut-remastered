...
async fn test_multiple_characters_per_account() {
    ...
    // Register + login
    client
        .send(&protocol::auth::ClientMsg::Register {
    ...
    client
        .send_char(&character::ClientMsg::CharacterCreate {
            name: "Hero One".to_string(),
            race: "elf".to_string(),
            class: "mage".to_string(),
            gender: "female".to_string(),
            ability_scores: [10, 14, 13, 15, 12, 8],
    ...
    assert!(matches!(
        resp1,
        character::ServerMsg::CharacterCreateOk { .. }
    ));

    client
        .send_char(&character::ClientMsg::CharacterCreate {
            name: "Hero Two".to_string(),
...
async fn test_invalid_race_rejected() {
    ...
    assert!(
        matches!(resp, character::ServerMsg::CharacterCreateFail { .. }),
        "expected CharacterCreateFail, got {:?}",
        resp
    );
}
...
async fn test_invalid_point_buy_rejected() {
    ...
    client
        .send_char(&character::ClientMsg::CharacterCreate {
            ...
            class: "warrior".to_string(),
            gender: "male".to_string(),
            ability_scores: [15, 15, 15, 15, 15, 15],
...
async fn test_vitals_on_character_select() {
    ...
    // Select character
    client
        .send_char(&character::ClientMsg::CharacterSelect {
...
async fn test_character_bio() {
    ...
    // Set bio
    client
        .send_world(&protocol::world::ClientMsg::Bio {
            text: "A wanderer from distant lands.".to_string(),
        })
        .await;
    let bio_resp = client.recv_world().await;
    assert!(
        matches!(bio_resp, protocol::world::ServerMsg::BioOk),
        "expected BioOk, got {:?}",
        bio_resp
    );
